...
#第三列有数字和字符串混合的类型所以为其指定列的类型
dtypes = {
    3: str
}

#为训练集和测试集读取数据
train_data = pd.read_csv(r'train.csv', dtype=dtypes)

test_data = pd.read_csv(r'test.csv', dtype=dtypes)
...
#删除 'bad_good' 列，以便后续的特征处理和模型训练只使用除目标列之外的特征。
...
x_train, x_test, y_train, y_test = train_test_split(train_data, train_data_target, test_size=0.2)
...
XGB = XGBClassifier(nthread=4,  # 含义：nthread=-1时，使用全部CPU进行并行运算（默认）, nthread=1时，使用1个CPU进行运算。
                    ...
                    n_estimators=40,  # 含义：总共迭代的次数，即决策树的个数

                    max_depth=6,  # 含义：树的深度，默认值为6，典型值3-10。调参：值越大，越容易过拟合；值越小，越容易欠拟合

                    gamma=0,  # 含义：惩罚项系数，指定节点分裂所需的最小损失函数下降值。

                    subsample=0.6,  # 含义：训练每棵树时，使用的数据占全部训练集的比例。默认值为1，典型值为0.5-1。调参：防止过拟合

                    colsample_bytree=0.7)  # 训练每棵树时，使用的特征占全部特征的比例。默认值为1，典型值为0.5-1。调参：防止过拟合


score = cross_val_score(XGB, train_data, train_data_target, cv=6).mean()

print("在训练集上的平均6折交叉验证分数:", score)
